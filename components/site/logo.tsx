 ...
 * Brand red leads into the first letters, then settles into `color`.
 * `gradientId` must be unique per instance: gradient stops resolve currentColor
 * where they're defined, so sharing one id would leak another theme's color.
 */
export function LogoWordmark({
  ...
  color = "currentColor",
  gradientId,
}: SvgSizeProps & { color?: string; gradientId: string }) {
  return (
    <svg
      width={width}
      height={height}
    ...
    >
      <defs>
        <linearGradient id={gradientId} gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="855.4" y2="0">
          <stop offset="0" stopColor={LOGO_RED} />
...
export function Logo({
  ...
  const gradientId = `logo-wm-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;
  return (
    <span className={cn("inline-flex items-center gap-3 text-foreground", className)}>
      <LogoMark className="h-[22px] w-auto" />
      {showWordmark && (
        <>
          <LogoWordmark className="h-[13px] w-auto" gradientId={gradientId} />
